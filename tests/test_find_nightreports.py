...
class FindReportsTestCase(unittest.IsolatedAsyncioTestCase):
    async def test_find_reports(self) -> None:
        num_reports = 12
        num_edited = 6  # Must be at least 4 in order to test ranges.
        async with create_test_client(
            num_reports=num_reports, num_edited=num_edited
        ) as (
            client,
            reports,
        ):
            ...
            for field, prefix in itertools.product(fields, ("", "-")):
                ...
                while len(paged_reports) < len(reports):
                    ...
                    response = await client.get(
                        "/nightreport/reports", params=find_args
                    )
